...
# Re-use the trajectory's atm_vec + aero + boost_thrust by importing:
import sys, os
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from powered_flight_trajectory_code_v4 import (
    atm_vec, aero, boost_thrust, boost_isp,
    G0, RE, M_STRUCT, M0 as M0_CONST,
)

class BoostODEPath(om.ExplicitComponent):
    ...
    def compute(self, I, O):
        ...
        V = np.clip(I['V'],  50., 4_000.)
        gm, m, al = I['gamma'], np.clip(I['m'], M_STRUCT, M0_CONST+10), I['alpha']
        rho,_,_,a = atm_vec(h)
        Mn = V/a
        CL,CD,L,D,q = aero(Mn, al, rho, V)
        T  = np.array([boost_thrust(Mi, hi) for Mi,hi in zip(Mn,h)])
        isp= np.full_like(Mn, boost_isp())
...
class DescentODEPath(om.ExplicitComponent):
    def initialize(self): self.options.declare('num_nodes', types=int)
    def setup(self):
        nn = self.options['num_nodes']
        for n,u in [('x_range','m'),('h','m'),('V','m/s'),
            ...
            self.add_input(n, val=np.zeros(nn), units=u)
        for n,u in [('x_range_dot','m/s'),('h_dot','m/s'),('V_dot','m/s**2'),
                    ('gamma_dot','rad/s'),('m_dot','kg/s'),('q','Pa')]:
            self.add_output(n, val=np.zeros(nn), units=u)
        self.declare_partials('*','*', method='fd')
    def compute(self, I, O):
        h=np.clip(I['h'],5.,79_000.); V=np.clip(I['V'],50.,4_000.)
        gm,m,al = I['gamma'], np.clip(I['m'], M_STRUCT*0.9, M0_CONST+10), I['alpha']
        rho,_,_,a=atm_vec(h); Mn=V/a
        CL,CD,L,D,q=aero(Mn,al,rho,V); g=G0*(RE/(RE+h))**2; ar=np.deg2rad(al)
        O['x_range_dot']=V*np.cos(gm); O['h_dot']=V*np.sin(gm)
        O['V_dot']      =-D/m - g*np.sin(gm)
        O['gamma_dot']  =(L/(m*V) - (g/V-V/(RE+h))*np.cos(gm))
        O['m_dot']      =np.zeros_like(h); O['q']=q
