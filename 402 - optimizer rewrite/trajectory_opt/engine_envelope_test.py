...
class EnvelopeSpec:
    ...
    min_combustor_volume_m3: float = 0.01
    min_thrust_N: float = 4000
    phi_for_thrust: float = 0.8
...
def _candidate_grid(base: Design, spec: EnvelopeSpec) -> Iterable[Design]:
    ...
    knobs = {
        #"design_M0":         (4.5, 5.0),
        "design_mdot_kgs":   (6.0, 7.0, 8.0),
        #"LE_angle_deg":      (3.5, 4.5),
        "diffuser_AR":       (2.0, 3.0, 4.0),
        "ramp_sep_margin":   (0.1, 0.15, 0.2),
...
def _refined_grid(seed: Design, spec: EnvelopeSpec) -> Iterable[Design]:
    ...
    knobs = {
        ...
        "diffuser_AR":       around(seed.diffuser_AR,       0.125, 2.0,  3.0),
        "ramp_sep_margin":   around(seed.ramp_sep_margin,   0.025, 0.1, 0.2),
        "kantrowitz_margin": around(seed.kantrowitz_margin, 0.025, 0.8, 0.90),
    }
    names = tuple(knobs.keys())
    for values in itertools.product(*(knobs[name] for name in names)):
