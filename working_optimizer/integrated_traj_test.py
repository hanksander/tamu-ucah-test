...
class RocketBoostEOM(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        try:
            X_scaled = self.scalers['X_scaler'].transform(predictor_input)
            CL = self.model_list[1].predict(X_scaled)
            CD = self.model_list[0].predict(X_scaled)
            q_dot = self.model_list[3].predict(X_scaled)
...
def run_dymos_optimization(path, plotting):
...
### first get the CBaero models ###
    ...
    phase = dm.Phase(
        ...
        transcription=dm.GaussLobatto(num_segments=10, order=7)
    )
...
#     plt.tight_layout(rect=[0, 0.03, 1, 0.95])
#     plt.savefig('./rocket_boost_trajectory_results.png')
#     plt.show()
...
#     phase.add_path_constraint('alpha', lower=np.deg2rad(-2), upper=np.deg2rad(8), ref=0.1)
#     phase.add_path_constraint('g_load', upper=15.0, ref=10.0)  # Max g-load constraint
#     phase.add_path_constraint('q_dot', upper=12000000.0, ref=100000.0)  # Max heat flux constraint
